    ...
    if (!$first_category && !empty($product_cats)) {
        foreach ($product_cats as $cat) {
            if (strtoupper($cat->name) !== 'JUGUETERIA' && strtoupper($cat->name) !== 'SIN CATEGORIZAR') {
                ...
                // DEBUG: descomentar la siguiente línea para ver el slug
                // error_log("DEBUG Category: {$cat->name} => slug: {$cat->slug}");
                break;
            }
        }
    }
...
<div class="<?= apply_filters('bootscore/class/woocommerce/col', 'col-6 col-md-6 col-lg-4 col-xxl-3'); ?>">
  <div <?php wc_product_class( apply_filters( 'bootscore/class/woocommerce/product/card', 'card h-100 position-relative product-card-improved' ), $product ); ?>>
    ...
    <?php if (!empty($edades)): ?>
    <div class="product-badge-age position-absolute top-0 start-0 p-2 d-flex flex-column gap-2" style="z-index: 20;">
      <?php foreach ($edades as $edad): ?>
        <span class="badge" style="background-color: <?php echo esc_attr($edad['color']); ?> !important; color: white !important; font-weight: 700; font-size: 0.7rem; padding: 0.35rem 0.7rem; box-shadow: 0 2px 8px rgba(0,0,0,0.25); border-radius: 50rem; white-space: nowrap;">
          <?php echo esc_html($edad['display']); ?>
        </span>
      <?php endforeach; ?>
    </div>
    <?php endif; ?>

    <!-- Badges superiores derecha (Oferta, Nuevo, Envío Gratis) -->
    ...
    <div class="<?= apply_filters('bootscore/class/woocommerce/product/card/card-body', 'card-body d-flex flex-column'); ?>">
      ...
      <div class="product-meta-badges d-flex flex-wrap gap-2 mb-3">
        <span class="badge rounded-pill d-flex align-items-center gap-1"
           style="background-color: <?php echo esc_attr($category_data['color']); ?> !important; color: white !important; font-weight: 700; font-size: 0.65rem; padding: 0.4rem 0.6rem; box-shadow: 0 2px 6px rgba(0,0,0,0.12); width: fit-content; cursor: default;">
