...
$empty_wishlist_text           = isset( $params['empty_wishlist_text'] ) ? $params['empty_wishlist_text'] : __( 'Tu lista de deseos está vacía.', 'wish-list-for-woocommerce' );
...
if( $work_with_cache && get_option( 'woocommerce_myaccount_page_id' ) == $current_page_id ){
	$structure = get_option( 'permalink_structure', '' );
	if ( $structure == '' ) {
		$wish_list_permalink = untrailingslashit( $wish_list_permalink ) . '&' . $wl_tab_slug;
		$query_string        = '&';
...
.alg-wc-wl-tablink.active {
	...
	box-shadow: 0 4px 12px rgba(238,40,91,0.25);
}
...
.alg-wc-delete-wishlist .copy-wishlist:hover {
	background: var(--secondary-color);
	color: #fff;
	border-color: var(--secondary-color);
}

.alg-wc-delete-wishlist .delete-customized-wishlist {
	background: #fff;
	color: #DC3232;
	border-color: #DC3232;
}

.alg-wc-delete-wishlist .delete-customized-wishlist:hover {
	background: #DC3232;
	color: #fff;
}
...
.alg-wc-wl-view-table .product-stock {
	font-weight: 600;
	padding: 6px 12px;
...
.alg-wc-wl-view-table .add-to-cart-btn .button:hover {
	transform: translateY(-2px);
	box-shadow: 0 6px 16px rgba(238,40,91,0.3);
}

/* Botón Eliminar */
.alg-wc-wl-view-table .product-removal .remove {
...
.alg-wc-wl-view-table .product-removal .remove:hover {
	background: var(--primary-color);
	color: #fff;
}

/* Cantidad */
.alg-wc-wl-view-table .product-qty input[type="number"] {
	width: 70px;
	border: 2px solid var(--light-bg);
	border-radius: 0.8rem;
	padding: 8px;
...
.alg-wc-wl-view-table .product-qty input[type="number"]:focus {
	border-color: var(--primary-color);
	outline: none;
}
...
@media (max-width: 768px) {
	...
	.alg-wc-wl-view-table tbody td:before {
...
<div class="alg-wc-wl-view-table-container <?php echo $work_with_cache ? 'ajax-loading' : '' ?>">
		...
		if ( is_array( $wishlist_list ) ) { ?>
			<div class="alg-wc-wl-tab">
				...
				if ( $is_email ) {
					if ( $current_tab_id == '' ) {
						?>
						<button class="alg-wc-wl-tablink <?php if ( $current_tab_id == '' ) { echo "active"; } ?>" onclick="location.href='<?php echo esc_url( $wish_list_permalink ); ?>'"><?php echo esc_html( $default_wl_text ); ?></button>
						<?php
					}
				} else {
					?>
				...
				foreach ( $wishlist_list as $k => $list ) {
					...
					if ( $is_email ) {
						if ( $tab_id == $current_tab_id ) {
							?>
							<button class="alg-wc-wl-tablink <?php echo esc_attr( $active ); ?>" onclick="location.href='<?php echo esc_url( $wish_list_permalink ) . esc_attr( $query_string ) . 'wtab=' . intval( $tab_id ); ?>'"><?php echo esc_html( $list ); ?></button>
							<?php
						}
					} else {
						?>
	...
	<?php if ( $alg_wc_wl_dropdown_sorting == 'yes' ) { ?>
		<form action="<?php echo esc_url( $wish_list_permalink ); ?>" method="GET">
			<select name="alg_wc_wl_orderby" class="alg_wc_wl_orderby" aria-label="Orden de lista" onchange="this.form.submit()">
				<option value="" <?php selected( '', $alg_wc_wl_orderby ); ?>><?php _e( 'Orden por defecto', 'wish-list-for-woocommerce' ); ?></option>
				<option value="name-asc" <?php selected( 'name-asc', $alg_wc_wl_orderby ); ?>><?php _e( 'Nombre A - Z', 'wish-list-for-woocommerce' ); ?></option>
				<option value="name-desc" <?php selected( 'name-desc', $alg_wc_wl_orderby ); ?>><?php _e( 'Nombre Z - A', 'wish-list-for-woocommerce' ); ?></option>
				<option value="date-asc" <?php selected( 'date-asc', $alg_wc_wl_orderby ); ?>><?php _e( 'Más recientes', 'wish-list-for-woocommerce' ); ?></option>
				<option value="date-desc" <?php selected( 'date-desc', $alg_wc_wl_orderby ); ?>><?php _e( 'Más antiguos', 'wish-list-for-woocommerce' ); ?></option>
				<option value="price-asc" <?php selected( 'price-asc', $alg_wc_wl_orderby ); ?>><?php _e( 'Precio: menor a mayor', 'wish-list-for-woocommerce' ); ?></option>
				<option value="price-desc" <?php selected( 'price-desc', $alg_wc_wl_orderby ); ?>><?php _e( 'Precio: mayor a menor', 'wish-list-for-woocommerce' ); ?></option>
				<option value="sku-asc" <?php selected( 'sku-asc', $alg_wc_wl_orderby ); ?>><?php _e( 'SKU A - Z', 'wish-list-for-woocommerce' ); ?></option>
				<option value="sku-desc" <?php selected( 'sku-desc', $alg_wc_wl_orderby ); ?>><?php _e( 'SKU Z - A', 'wish-list-for-woocommerce' ); ?></option>
			</select>
	...
	<div class="alg-wc-delete-wishlist">
		<?php if ( $user_id > 0 && $alg_wc_wl_duplicate_option == 'yes' ): ?>
			<a href="javascript:;" data-page="<?php echo esc_attr( $page ); ?>"
			   ...
			   title="<?php _e( 'Copiar Lista', 'wish-list-for-woocommerce' ); ?>"
			   rel="nofollow">
			   <i class="bi bi-files"></i> <?php _e( 'Copiar Lista', 'wish-list-for-woocommerce' ); ?>
			</a>
		<?php endif; ?>
		<?php if ( $current_tab_id > 0 && $alg_wc_wl_delete_wishlist_option == true ): ?>
			<a href="javascript:;" data-page="<?php echo $page; ?>"
			   ...
			   title="<?php _e( 'Eliminar Lista', 'wish-list-for-woocommerce' ); ?>"
			   rel="nofollow">
			   <i class="bi bi-trash"></i> <?php _e( 'Eliminar Lista', 'wish-list-for-woocommerce' ); ?>
			</a>
		<?php endif; ?>
	</div>
	...
	<?php if ( $the_query != null && $the_query->have_posts() ) : ?>
		...
		<table <?php echo esc_attr( $email_table_params ); ?> class="alg-wc-wl-view-table">
			...
			<tr>
				...
				<th colspan="<?php echo ! $show_product_thumb ? '1' : '2'; ?>" class="product"><?php _e( 'Producto', 'wish-list-for-woocommerce' ); ?></th>

				<?php if ( $show_price ) : ?>
					<th class="product-price"><?php _e( 'Precio', 'wish-list-for-woocommerce' ); ?></th>
				<?php endif; ?>

				<?php if ( $show_prod_category ) : ?>
					<th class="product-category"><?php _e( 'Categoría', 'wish-list-for-woocommerce' ); ?></th>
				<?php endif; ?>
				...
				<?php if ( $description ) : ?>
					<th class="product-description"><?php _e( 'Descripción', 'woocommerce' ); ?></th>
				<?php endif; ?>
				...
				<?php if ( $quantity ) : ?>
					<th class="product-qty"><?php _e( 'Cantidad', 'woocommerce' ); ?></th>
				<?php endif; ?>
				...
				<?php if ( $show_add_to_cart_btn ) : ?>
					<th class="add_to_cart_btn"><?php _e( 'Agregar', 'woocommerce' ); ?></th>
				<?php endif; ?>

				<?php if ( $can_remove_items ) : ?>
					<th class="product-removal"><?php _e( 'Quitar', 'wish-list-for-woocommerce' ); ?></th>
				<?php endif; ?>
			</tr>
			</thead>
			<tbody>
			<?php while ( $the_query->have_posts() ) : $the_query->the_post(); ?>
				...
				<tr data-product-id="<?php echo esc_attr( $data_product_id ) ?>">
					<?php if ( $drag_drop ) : ?>
						<td data-title="<?php _e( 'Arrastrar', 'wish-list-for-woocommerce' ); ?>" class="drag-drop">
					...
					<?php if ( $arrow_sorting ) : ?>
						<td data-title="<?php _e( 'Ordenar', 'wish-list-for-woocommerce' ); ?>" class="arrow-sorting">
					...
					<?php if ( $show_product_thumb ) : ?>
						<td data-title="<?php _e( 'Imagen', 'wish-list-for-woocommerce' ); ?>" class="product-thumbnail">
							<a href="<?php echo esc_url( $product->get_permalink() ); ?>">
								<?php if ( $is_email ) { ?>
									<?php echo wp_kses_post( $product->get_image( apply_filters( 'alg_wc_wl_product_image_size', 'woocommerce_gallery_thumbnail', 'email' ) ) ); ?>
								<?php } else { ?>
									<?php echo wp_kses_post( $product->get_image( apply_filters( 'alg_wc_wl_product_image_size', 'woocommerce_thumbnail', '' ) ) ); ?>
								<?php } ?>
							</a>
						</td>
					<?php endif; ?>

					<td data-title="<?php _e( 'Producto', 'wish-list-for-woocommerce' ); ?>" class="product-name">
						...
						<?php if ( is_a( $product, 'WC_Product_Variation' ) && 'yes' === get_option( Alg_WC_Wish_List_Settings_List::OPTION_SAVE_ATTRIBUTES, 'yes' ) ) {
							foreach ( $product->get_attributes() as $variation_attribute => $term_slug ) {
								...
								echo '<div style="font-size:13px;color:#666;"><strong>' . esc_html( $attribute_label ) . ':</strong> ' . esc_html( $attribute_value ) . '</div>';
							}
						} ?>
					</td>

					<?php if ( $show_price ) : ?>
						<td data-title="<?php _e( 'Precio', 'wish-list-for-woocommerce' ); ?>" class="product-price">
							<?php echo wp_kses_post( $product->get_price_html() ); ?>
						</td>
					<?php endif; ?>

					<?php if ( $show_prod_category ) : ?>
						<td data-title="<?php _e( 'Categoría', 'wish-list-for-woocommerce' ); ?>" class="product-cat">
					...
					<?php if ( $description ) : ?>
						<td data-title="<?php _e( 'Descripción', 'wish-list-for-woocommerce' ); ?>"
					...
					<?php if ( $show_stock ) : ?>
						<td data-title="<?php _e( 'Stock', 'woocommerce' ); ?>" class="product-stock">
							<?php echo empty( $availability = $product->get_availability()['availability'] ) ? _e( 'Disponible', 'woocommerce' ) : esc_html( $availability ); ?>
						</td>
					<?php endif; ?>
					...
					<?php if ( $quantity ) : ?>
						<td data-title="<?php _e( 'Cantidad', 'wish-list-for-woocommerce' ); ?>"
							class="product-qty">
					...
					<?php if ( $show_add_to_cart_btn ) : ?>
						<td data-title="<?php _e( 'Agregar', 'woocommerce' ); ?>"
					...
					<?php if ( $can_remove_items ) : ?>
						<td data-title="<?php _e( 'Quitar', 'wish-list-for-woocommerce' ); ?>" class="product-removal">
							<?php
